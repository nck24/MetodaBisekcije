    ...
    class Program
    {
        static void Main(string[] args){
            ...
            for (int i = 0; i < (MaxStopnja(koeficijenti) - 2); i++){
                koefOdvodov.Add(KoeficientiOdvoda(koefOdvodov[koefOdvodov.Count - 1]));
            }

            List<double> intervaliNicel = new List<double>(); //Vrednost za intervale kjer so potencialne nicle
            if (NicleKvadratneFunk(koefOdvodov.Last())[0] != null){ //Pogledamo ce slucajno kvadratna funk (zadnji odvod) nima nicel
                ...
                Console.WriteLine("Ta polinom nima nicel");
            }
