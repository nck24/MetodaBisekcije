...
namespace MetodaBisekcije{
    class Program
    {
        static void Main(string[] args){
            ...
            Console.Write("Na koliko decimalk natancno (max 13) : ");
        ...
        static double[] NiclePolinoma(double[] polinom, int natancnost){
            if (polinom.Length == 2){
                ...
                if (diskriminanta > 0){
                    ...
                    nicli[0]= Math.Round((((double)polinom[1] * (-1)) - Math.Sqrt(diskriminanta)) / ((double)polinom[0]*2), natancnost);
                    nicli[1]= Math.Round((((double)polinom[1] * (-1)) + Math.Sqrt(diskriminanta)) / ((double)polinom[0]*2), natancnost);
                ...
                List<double> intervali = new List<double>(NiclePolinoma(OdvodPolinoma(polinom), 13));
                ...
                for (int i = 0; i < intervali.Count - 1; i++){
                    if (Math.Abs(VrednostVX(polinom, intervali[i])) <= 0.0000000000001D){
                        if (intervali[i] == 0)
                            nicle.Add(Math.Abs(intervali[i]));
                        else
        ...
        static double Bisekcija(double[] polinom, double x1, double x2, int natancnost){
            ...
            double a = Math.Round(interval[0], natancnost);
            double b = Math.Round(interval[1], natancnost);
            ...
            while (a != b){
                ...
                a = Math.Round(interval[0], natancnost);;
                b = Math.Round(interval[1], natancnost);
            }
