...
namespace MetodaBisekcije{
    class Program
    {
        static void Main(string[] args){
            ...
            Console.Write("Na koliko decimalnih mest hoces imeti nicle (max 12) : ");
        ...
        static double[] KoeficientiPolinoma(){
            ...
            while (pozZnaka != -1){
                if (polinom[0] == 'x'){
                    koeficienti[StopnjaPoli(polinom)] = 1;
        ...
        static double?[] NicleKvadratneFunk(double[] koefKvadratneFunk){
            ...
            double determinanta = Math.Pow(koefKvadratneFunk[1], 2) - 4 * koefKvadratneFunk[0] * koefKvadratneFunk[2];
            if (determinanta < 0){
                nicli[0] = null;
                nicli[1] = null;
                return nicli;
            }else{
                nicli[0] = (-koefKvadratneFunk[1] + Math.Sqrt(determinanta)) / (2 * koefKvadratneFunk[0]);
                nicli[1] = (-koefKvadratneFunk[1] - Math.Sqrt(determinanta)) / (2 * koefKvadratneFunk[0]);
                return nicli;
            }
        }
